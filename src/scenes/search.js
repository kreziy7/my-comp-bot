...
searchScene.enter(async (ctx) => {
  await ctx.reply(ctx.t('search_prompt'), { reply_markup: { remove_keyboard: true } });
});
...
searchScene.on('text', async (ctx) => {
  ...
  if (results.length === 0) {
    await ctx.reply(ctx.t('search_no_results'));
    return;
  }
  await ctx.reply(ctx.t('search_results', { count: results.length }), searchResultsKb(results));
  await ctx.scene.leave();
